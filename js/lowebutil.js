...
function cookieGet(cookieRequest, callback) {
  if (!isElectron()) {
    const { url, name } = cookieRequest
    const cookie = Cookies.get(name, { domain: url })
...
function cookieSet(cookie, callback) {
  if (!isElectron()) {
    const { name, value, url, expirationDate} = cookie
    const cookieRes = Cookies.set(name, value, { domain: url, expires: expirationDate })
    callback(cookieRes)
    return
...
function cookieRemove(cookie, callback) {
  if (!isElectron()) {
    ...
    Cookies.remove(name, { domain: url })
    callback()
    return
