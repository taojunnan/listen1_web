...
class qq {
  ...
  static qq_show_toplist(offset) {
    ...
    const url =
      cqq + '/v8/fcg-bin/fcg_myqq_toplist.fcg?g_tk=5381&inCharset=utf-8&outCharset=utf-8&notice=0&format=json&uin=0&needNewCode=1&platform=h5';
        ...
        axios.get(url).then((response) => {
          ...
          response.data.data.topList.forEach((item) => {
            const playlist = {
              ...
              source_url: `${baseqq}/n/yqq/toplist/${item.id}.html`,
              title: item.topTitle,
            };
  ...
  static show_playlist(url) {
    ...
    const target_url =
      cqq + '/splcloud/fcgi-bin/fcg_get_diss_by_tag.fcg' +
      ...
      success: (fn) => {
        axios.get(target_url).then((response) => {
          ...
          const playlists = data.data.list.map((item) => ({
            ...
            source_url: `${baseqq}/n/ryqq/playlist/${item.dissid}`,
          }));
  ...
  static qq_convert_song(song) {
    const d = {
      ...
      source: 'qq',
      source_url: `${baseqq}/#type=song&mid=${song.songmid}&tpl=yqq_song_detail`,
  ...
  static qq_convert_song2(song) {
    const d = {
      ...
      source: 'qq',
      source_url: `${baseqq}/#type=song&mid=${song.mid}&tpl=yqq_song_detail`,
      url: '',
    };
    return d;
  }
  ...
  static get_toplist_url(id, period, limit) {
    return `${uqq}/cgi-bin/musicu.fcg?format=json&inCharset=utf8&outCharset=utf-8&platform=yqq.json&needNewCode=0&data=${encodeURIComponent(
      JSON.stringify({
        comm: {
          cv: 1602,
          ct: 20,
  ...
  static get_periods(topid) {
    const periodUrl = cqq + '/node/pc/wk_v15/top.html';
    const regExps = {
      periodList:
        ...
        this.get_periods(list_id).then((listPeriod) => {
          ...
          axios.get(target_url).then((response) => {
            ...
            const info = {
              ...
              source_url: `${baseqq}/n/yqq/toplist/${list_id}.html`,
            };
            return fn({
              tracks,
              info,
  ...
  static qq_get_playlist(url) {
    ...
    return {
      success: (fn) => {
        const target_url =
          iqq + '/qzone-music/fcg-bin/fcg_ucc_getcdinfo_' +
        ...
        axios.get(target_url).then((response) => {
          ...
          const info = {
            ...
            source_url: `${baseqq}/n/ryqq/playlist/${list_id}`,
          };
  ...
  static qq_album(url) {
    ...
    return {
      success: (fn) => {
        const target_url =
          iqq + '/v8/fcg-bin/fcg_v8_album_info_cp.fcg' +
        ...
        axios.get(target_url).then((response) => {
          ...
          const info = {
            ...
            source_url: `${baseqq}/#type=album&mid=${album_id}`,
          };
  ...
  static qq_artist(url) {
    ...
    return {
      success: (fn) => {
        const target_url = `${uqq}/cgi-bin/musicu.fcg?format=json&loginUin=0&hostUin=0inCharset=utf8&outCharset=utf-8&platform=yqq.json&needNewCode=0&data=${encodeURIComponent(
          JSON.stringify({
            comm: {
              ct: 24,
              cv: 0,
        ...
        axios.get(target_url).then((response) => {
          ...
          const info = {
            ...
            source_url: `${baseqq}/#type=singer&mid=${artist_id}`,
          };
  ...
  static search(url) {
    ...
    const target_url = `${uqq}/cgi-bin/musicu.fcg`;
        ...
        axios.post(target_url, query).then((response) => {
          ...
          if (searchType === '0') {
            ...
            result = data.req.data.body.songlist.list.map((info) => ({
              ...
              source: 'qq',
              source_url: `${baseqq}/n/ryqq/playlist/${info.dissid}`,
              img_url: info.imgurl,
  ...
  static lyric(url) {
    ...
    const target_url =
      iqq + '/lyric/fcgi-bin/fcg_query_lyric_new.fcg?' +
  ...
  static get_playlist_filters() {
    const target_url =
      cqq + '/splcloud/fcgi-bin/fcg_get_diss_tag_conf.fcg' +
      `?picmid=1&rnd=${Math.random()}&g_tk=732560869` +
      '&loginUin=0&hostUin=0&format=json&inCharset=utf8&outCharset=utf-8' +
      '&notice=0&platform=yqq.json&needNewCode=0';
  ...
  static get_user_by_uin(uin, callback) {
    const infoUrl = `${uqq}/cgi-bin/musicu.fcg?format=json&&loginUin=${uin}&hostUin=0inCharset=utf8&outCharset=utf-8&platform=yqq.json&needNewCode=0&data=${encodeURIComponent(
      JSON.stringify({
        comm: { ct: 24, cv: 0 },
        vip: {
  ...
  static get_user_created_playlist(url) {
    ...
    const target_url = `${cqq}/rsc/fcgi-bin/fcg_user_created_diss?cv=4747474&ct=24&format=json&inCharset=utf-8&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=1&uin=${user_id}&hostuin=${user_id}&sin=0&size=${size}`;
            ...
            if (item.dir_show === 0) {
              ...
              if (item.diss_name === '我喜欢') {
                playlist = {
                  ...
                  source_url: `${baseqq}/n/ryqq/playlist/${item.tid}`,
  ...
  static get_user_favorite_playlist(url) {
    ...
    const target_url = `${cqq}/fav/fcgi-bin/fcg_get_profile_order_asset.fcg`;
    const data = {
      ct: 20,
      cid: 205360956,
      userid: user_id,
        ...
        axios.get(target_url).then((response) => {
          ...
          response.data.recomPlaylist.data.v_hot.forEach((item) => {
            const playlist = {
              ...
              source_url: `${baseqq}/n/ryqq/playlist/${item.content_id}`,
              title: item.title,
            };
            playlists.push(playlist);
          });
