...
const kugouBaseUrl = '/kugou'
const kugouSearchUrl = '/kugousearch'
const kugoumUrl = '/kugoum'
const kugouMobileUrl = '/kugoumobile'
const kugouApiUrl = 'kugouapi'
...
class kugou {
  static kg_convert_song(song) {
    const track = {
      ...
      source: 'kugou',
      source_url: `${kugouBaseUrl}/song/#hash=${song.FileHash}&album_id=${song.AlbumID}`,
      img_url: '',
  ...
  static kg_render_search_result_item(index, item, params, callback) {
    ...
    // Add singer img
    const url = `${kugouBaseUrl}/yy/index.php?r=play/getdata&hash=${track.lyric_url}`;
  ...
  static search(url) {
    ...
    return {
      success: (fn) => {
        const target_url = `${kugouSearchUrl}/song_search_v2?keyword=${keyword}&page=${curpage}`;
        axios
          .get(target_url)
          .then((response) => {
            const { data } = response;
  ...
  static kg_render_playlist_result_item(index, item, params, callback) {
    ...
    let target_url = `${kugoumUrl}/app/i/getSongInfo.php?cmd=playInfo&hash=${hash}`;
    const track = {
      id: `kgtrack_${hash}`,
      title: '',
      artist: '',
      ...
      source: 'kugou',
      source_url: `${kugouBaseUrl}/song/#hash=${hash}&album_id=${item.album_id}`,
      img_url: '',
      lyric_url: hash,
    };
    // Fix song info
    axios.get(target_url).then((response) => {
      ...
      // Fix album
      target_url = `${kugouMobileUrl}/api/v3/album/info?albumid=${item.album_id}`;
  ...
  static kg_get_playlist(url) {
    return {
      success: (fn) => {
        ...
        const target_url = `${kugoumUrl}/plist/list/${list_id}/?json=true`;
        ...
        axios.get(target_url).then((response) => {
          ...
          const info = {
            ...
            source_url: `${kugouBaseUrl}/yy/special/single/${data.info.list.specialid}.html`
          };
  ...
  static kg_render_artist_result_item(index, item, params, callback) {
    ...
    const track = {
      ...
      album_id: `kgalbum_${item.album_id}`,
      source: 'kugou',
      source_url: `${kugouBaseUrl}/song/#hash=${item.hash}&album_id=${item.album_id}`,
      img_url: '',
    ...
    const target_url = `${kugouBaseUrl}/yy/index.php?r=play/getdata&hash=${item.hash}`;
    axios
      .get(`${kugouMobileUrl}/api/v3/album/info?albumid=${item.album_id}`)
      .then((response) => {
        const { data } = response;
  ...
  static kg_artist(url) {
    return {
      success: (fn) => {
        ...
        let target_url = `${kugouMobileUrl}/api/v3/singer/info?singerid=${artist_id}`;
        axios.get(target_url).then((response) => {
          const { data } = response;
          const info = {
            ...
            source_url: `${kugouBaseUrl}/singer/${artist_id}.html`
          };
          target_url = `${kugouMobileUrl}/api/v3/singer/song?singerid=${artist_id}&page=1&pagesize=30`;
          axios.get(target_url).then((res) => {
            this.async_process_list(
              res.data.data.info,
  ...
  static bootstrap_track(track, success, failure) {
    ...
    let target_url = `${kugouApiUrl}/yy/index.php?r=play/getdata&callback=jQuery&mid=1&hash=${track_id}&platid=4`;
    if (album_id !== '') {
  ...
  static lyric(url) {
    ...
    let lyric_url = `${kugouApiUrl}/yy/index.php?r=play/getdata&callback=jQuery&mid=1&hash=${track_id}&platid=4&album_id=${album_id}`;
    const timstamp = +new Date();
  ...
  static kg_render_album_result_item(index, item, params, callback) {
    ...
    const track = {
      ...
      source: 'kugou',
      source_url: `${kugouBaseUrl}/song/#hash=${item.hash}&album_id=${album_id}`,
      img_url: '',
      // url: `xmtrack_${item.hash}`,
      lyric_url: item.hash,
    };
    // Fix other data
    const target_url = `${kugoumUrl}/app/i/getSongInfo.php?cmd=playInfo&hash=${item.hash}`;
    axios.get(target_url).then((response) => {
      const { data } = response;
      track.title = data.songName;
      track.artist = data.singerId === 0 ? '未知' : data.singerName;
  ...
  static kg_album(url) {
    return {
      success: (fn) => {
        ...
        let target_url = `${kugouMobileUrl}/api/v3/album/info?albumid=${album_id}`;
        ...
        axios.get(target_url).then((response) => {
          ...
          info = {
            ...
            source_url: `${kugouBaseUrl}/album/${data.data.albumid}.html`
          };

          target_url = `${kugouMobileUrl}/api/v3/album/song?albumid=${album_id}&page=1&pagesize=-1`;
          axios.get(target_url).then((res) => {
            this.async_process_list(
              res.data.data.info,
  ...
  static show_playlist(url) {
    ...
    const target_url = `${kugoumUrl}/plist/index&json=true&page=${page}`;
    return {
      success: (fn) => {
        axios.get(target_url).then((response) => {
          const { data } = response;
