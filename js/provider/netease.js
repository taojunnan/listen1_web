...
const interfaceBaseUrl = '/neteaseinterface';
...
class netease {
  ...
  static ne_show_toplist(offset) {
    ...
    const url = `${baseUrl}/weapi/toplist/detail`;
        ...
        axios.post(url, new URLSearchParams(data)).then((response) => {
          ...
          response.data.list.forEach((item) => {
            const playlist = {
              ...
              source_url: `${baseUrl}/#/playlist?id=${item.id}`,
              title: item.name,
            };
  ...
  static show_playlist(url) {
    ...
    let target_url = '';
    if (offset != null) {
      target_url = `${baseUrl}/discover/playlist/?order=${order}${filter}&limit=35&offset=${offset}`;
    } else {
      target_url = `${baseUrl}/discover/playlist/?order=${order}${filter}`;
    }
      ...
      success: (fn) => {
        axios.get(target_url).then((response) => {
          ...
          const result = list_elements.map((item) => ({
            ...
            source_url: `${baseUrl}/#/playlist?id=${getParameterByName(
              'id',
              item.getElementsByTagName('div')[0].getElementsByTagName('a')[0]
                .href
            )}`,
  ...
  static ng_render_playlist_result_item(index, item, callback) {
    const target_url = `${baseUrl}/weapi/v3/song/detail`;
    ...
    axios
      ...
      .then((response) => {
        ...
        const track = {
          ...
          source: 'netease',
          source_url: `${baseUrl}/#/song?id=${track_json.id}`,
  ...
  static ne_get_playlist(url) {
    ...
    return {
      success: (fn) => {
        ...
        const target_url = `${baseUrl}/weapi/v3/playlist/detail`;
        const d = {
          id: list_id,
          offset: 0,
          total: true,
        ...
        this.ne_ensure_cookie(() => {
          axios.post(target_url, new URLSearchParams(data)).then((response) => {
            ...
            const info = {
              ...
              source_url: `${baseUrl}/#/playlist?id=${list_id}`,
            };
  ...
  static bootstrap_track(track, success, failure) {
    const sound = {};
    const target_url = `${interfaceBaseUrl}/eapi/song/enhance/player/url`;
  ...
  static search(url) {
    ...
    const target_url = `${baseUrl}/api/search/pc`;
    // const target_url = `${baseUrl}/api/search/get/web`;
    ...
    return {
      success: (fn) => {
        axios
          .get(target_url, {params: new URLSearchParams(req_data)})
          .then((response) => {
            const { data } = response;
            let result = [];
            let total = 0;
            if (searchType === '0') {
              result = data.result.songs.map((song_info) => ({
                ...
                source: 'netease',
                source_url: `${baseUrl}/#/song?id=${song_info.id}`,
              ...
              result = data.result.playlists.map((info) => ({
                ...
                title: info.name,
                source: 'netease',
                source_url: `${baseUrl}/#/playlist?id=${info.id}`,
  ...
  static ne_album(url) {
    ...
    // use chrome extension to modify referer.
    const target_url = `${baseUrl}/api/album/${album_id}`;
      ...
      success: (fn) => {
        axios.get(target_url).then((response) => {
          ...
          const info = {
            ...
            source_url: `${baseUrl}/#/album?id=${data.album.id}`,
          };
  ...
  static ne_artist(url) {
    ...
    // use chrome extension to modify referer.
    const target_url = `${baseUrl}/api/artist/${artist_id}`;
      ...
      success: (fn) => {
        axios.get(target_url).then((response) => {
          ...
          const info = {
            ...
            source_url: `${baseUrl}/#/artist?id=${data.artist.id}`,
          };
  ...
  static lyric(url) {
    ...
    // use chrome extension to modify referer.
    const target_url = `${baseUrl}/weapi/song/lyric?csrf_token=`;
    const csrf = '';
    const d = {
      id: track_id,
      lv: -1,
  ...
  static login(url) {
    // use chrome extension to modify referer.
    let target_url = `${baseUrl}/weapi/login`;
    ...
    if (loginType === 'email') {
      ...
      target_url = `${baseUrl}/weapi/login/cellphone`;
  ...
  static get_user_playlist(url, playlistType) {
    ...
    const target_url = `${baseUrl}/api/user/playlist`;
  ...
  static get_recommend_playlist() {
    const target_url = `${baseUrl}/weapi/personalized/playlist`;
  ...
  static get_user() {
    const url = `${baseUrl}/api/nuser/account/get`;
